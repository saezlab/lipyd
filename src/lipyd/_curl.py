...
class cache_on(_global_context_on):
    """
    This is a context handler to turn on lipyd._curl.Curl() cache.
    As most of the methods use cache as their default behaviour,
    probably it won't change anything.

    Behind the scenes it sets the value of the `lipyd._curl.CACHE`
    module level variable to `True` (by default it is `None`).

    Example: ::

        import lipyd
        from lipyd import _curl as curl, moldb
...
class cache_off(_global_context_off):
    """
    This is a context handler to turn off lipyd._curl.Curl() cache.
...
class cache_print_on(_global_context_on):
    """
    This is a context handler which makes lipyd._curl.Curl() print
    verbose messages about its cache.

    Behind the scenes it sets the value of the `lipyd._curl.CACHEPRINT`
...
class cache_print_off(_global_context_off):
    """
    This is a context handler which stops lipyd._curl.Curl() to print
    verbose messages about its cache.
...
class cache_delete_on(_global_context_on):
    """
    This is a context handler which results lipyd._curl.Curl() deleting the
    ...
    Behind the scenes it sets the value of the `lipyd._curl.CACHEDEL`
    module level variable to `True` (by default it is `False`).
...
class cache_delete_off(_global_context_off):
    """
    This is a context handler which stops lipyd._curl.Curl() deleting the
...
class dryrun_on(_global_context_on):
    """
    This is a context handler which results lipyd._curl.Curl() to do all
    setup steps, but do not perform download or cache read.

    Behind the scenes it sets the value of the `lipyd._curl.DRYRUN`
    module level variable to `True` (by default it is `False`).
...
class dryrun_off(_global_context_off):
    """
    This is a context handler which results lipyd._curl.Curl() to
...
class preserve_on(_global_context_on):
    """
    This is a context handler which results lipyd._curl.Curl() to make
    ...
    Behind the scenes it sets the value of the `lipyd._curl.PRESERVE`
    module level variable to `True` (by default it is `False`).
...
class preserve_off(_global_context_off):
    """
    This is a context handler which avoids lipyd._curl.Curl() to make
...
class debug_on(_global_context_on):
    """
    This is a context handler which results lipyd._curl.Curl() to print
    debug information. 
    This is useful if you have some issue with `Curl`, and you want to
    see what`s going on.

    Behind the scenes it sets the value of the `lipyd._curl.DEBUG`
    module level variable to `True` (by default it is `False`).
...
class debug_off(_global_context_off):
    """
    This is a context handler which avoids lipyd._curl.Curl() to print
    debug information.
